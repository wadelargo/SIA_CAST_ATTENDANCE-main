...
<div class="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
    <a href="{{ url('/event/create') }}" class="btn btn-outline-success mb-3" type="button">
        Add New Event
    </a>
                ...
                <div class="card shadow-sm">
                    <div class="card-body text-center">
                        <div class="d-flex justify-content-center mb-3">
                            {!! QrCode::size(150)->generate($event->id . ' - ' . $event->title) !!}
                        </div>
                        <h5 class="card-title">{{ $event->title }}</h5>
                        <p class="card-text">
                            <strong>Description:</strong> {{ $event->description }}<br>
                            <strong>Date:</strong> {{ \Carbon\Carbon::parse($event->date)->format('M d, Y') }}
                        </p>
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <div class="btn-group">
                                <a href="{{ url('event/'.$event->id) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                            </div>
